...
# ================================================
#  Helper: Parse a Hotkey String into a keyboard.Key / KeyCode
# ================================================
def parse_hotkey_string(s: str):
    """
    Given a string like "f3", "F6", "a", "A", "space", "ctrl_l", etc.,
    return either a keyboard.Key (for named keys) or a KeyCode (for single characters).
    ...
    # Try matching against keyboard.Key attributes (e.g. 'f3', 'space', 'ctrl_l', etc.)
    try:
    ...
    # If it’s exactly one character, return a KeyCode for that char
    if len(s) == 1:
...
def hotkey_to_string(key_obj):
    """
    Convert a keyboard.Key or KeyCode back into a user-friendly string.
    E.g. Key.f3 -> "f3", KeyCode(char='a') -> "a".
    """
    if isinstance(key_obj, keyboard.Key):
        return key_obj.name  # e.g. "f3", "space"
...
class Macro:
    def __init__(self, name: str, trigger_key, button: str, key_to_send: str,
        ...
        """
        name        : A descriptive name for the macro
        trigger_key : keyboard.Key or KeyCode that will launch this macro
        button      : "left" / "middle" / "right" / "key"
        key_to_send : if button=="key", this is the string to send via pyautogui.press(...)
        n_clicks    : how many clicks or key-presses in this macro
        interval    : seconds between each click/keypress
        x_coord     : if not None, a fixed X coordinate to move/click at
        y_coord     : if not None, a fixed Y coordinate to move/click at
        start_delay : how many seconds to wait before the macro starts firing
        """
        self.name = name
    ...
    def display_name(self):
        """
        Return a string for the listbox: e.g. "MyMacro [f5]".
        """
    ...
    def from_dict(d):
        """
        Build a Macro instance from a dict that was read from JSON.
        """
...
# ================================================
#  The AutoClicker Logic Class (with Macro Support & Persistence)
# ================================================
class AutoClicker:
    def __init__(self, gui_reference):
        """
        gui_reference: a reference to the GUI object so that we can update status / total clicks.
        """
        self.gui = gui_reference

        # Master clicker settings (pulled from GUI periodically)
        self.clicks_per_second = 1.0         # “# clicks” now means clicks/sec for the master clicker
        self.trigger_key = keyboard.Key.f3   # default trigger (F3) for the master clicker
        self.stop_after_total = 0            # “stop at” (0 means infinite)

        self.mode = "press"                  # "press" or "toggle" for the master clicker
        self.button = "left"                 # "left", "middle", "right", or "key" for master clicker
        self.key_to_send = ""                # if button=="key", which key to send
        self.use_fixed_master = False        # if True, use fixed coords
        self.master_x = None
        self.master_y = None

        # Internal state for the master clicker:
        self._clicking = False               # True if the master clicker is actively clicking
        self._total_clicks_sent = 0          # increments each time ANY click/keypress is sent
        self._toggle_thread = None

        # Macro storage (list of Macro instances)
        self.macros = []

        # Load macros from disk, if present
        self._load_macros_from_disk()

        # Start the global listener immediately (catches both master trigger + macro triggers)
    ...
    # ----------------------------------------
    #  Persistence: Load/Save Macros
    # ----------------------------------------
    def _load_macros_from_disk(self):
        """
        If macros.json exists in the current working directory, read it and populate self.macros.
        """
        try:
    ...
    def _save_macros_to_disk(self):
        """
        Write self.macros (converted to dicts) into macros.json.
        """
        try:
    ...
    def _on_key_press(self, key):
        """
        Called when *any* key is pressed. We check in this order:
          1) If it matches a macro's trigger_key → launch that macro in its own thread.
          2) Else if it matches the master clicker's trigger_key:
               • In "press" mode: start clicking (until key is released).
               • In "toggle" mode: flip continuous clicking on/off.
        """
        # 1) Check macros first:
        for macro in self.macros:
            if key == macro.trigger_key:
                # Launch that macro in a background thread
                threading.Thread(target=self._run_macro, args=(macro,), daemon=True).start()
                return  # do not also start master clicker for this key press

        # 2) Otherwise, check the master clicker trigger:
    ...
    def _on_key_release(self, key):
        """
        In "press" mode (master clicker), releasing the trigger key means stop immediately.
        In "toggle" mode, releasing does nothing.
        """
        if self.mode == "press" and key == self.trigger_key:
            self._stop_continuous_master()

    # ----------------------------------------
    #  Master Clicker Logic (Continuous Clicking)
    # ----------------------------------------
    def _start_continuous_master(self):
        """
        For both Press & Toggle modes: begin a thread that clicks at self.clicks_per_second
        until toggled off or until we hit stop_after_total.
        """
    ...
    def _continuous_master_loop(self):
        """
        While self._clicking is True, send one click/keypress every (1 / clicks_per_second) seconds,
        stopping early if we've reached self.stop_after_total.
        """
        ...
        # Once exiting (toggled off or cap reached), go back to idle
    ...
    def _stop_continuous_master(self):
        """
        Called when either:
          • The user releases the trigger key in Press mode,
          • Presses the trigger again in Toggle mode,
          • Or clicks STOP! in the GUI.
        """
        self._clicking = False
        self.gui.set_status("idle")
    ...
    def _send_one_click_master(self):
        """
        Perform exactly one click (or keypress) for the master clicker,
        based on self.button / self.key_to_send, and optional fixed coordinate.
        """
    ...
    def _run_macro(self, macro: Macro):
        """
        Perform the sequence defined in `macro`.
        Each step is: (optional delay), move to fixed coordinate (if set), and click/keypress
        at the macro's chosen interval, up to macro.n_clicks times, unless the global stop cap is hit first.
        """
        # 1) Optional start delay
        ...
        # 3) After finishing the macro, revert to idle or master‐clicking state
        if self._clicking:
            self.gui.set_status("clicking")
    ...
    # ----------------------------------------
    #  Called When GUI Changes Settings
    # ----------------------------------------
    def update_settings_from_gui(self):
        """
        Read all of the GUI fields (StringVars, etc.) and sanitize them into internal variables.
        """
        # 1) "# clicks" (clicks per second for the master clicker)
        try:
        ...
        # 2) "trigger" key for the master clicker
        desired = self.gui.var_trigger_key.get().strip()
        pk = parse_hotkey_string(desired)
        if pk:
            self.trigger_key = pk
        # If invalid, we leave the old trigger intact (GUI will highlight in pink)
        ...
        # 4) Mode: "press" or "toggle"
        self.mode = self.gui.var_mode.get()

        # 5) Mouse vs. Key for the master clicker
        ...
        # 6) If button == "key", read the key
        self.key_to_send = self.gui.var_key_to_send.get().strip().lower()

        # 7) Fixed‐coordinate toggle for master clicker
    ...
    # ----------------------------------------
    #  Called When User Hits “STOP!” in GUI
    # ----------------------------------------
    def stop_immediately(self):
        """
        Force‐stop the master clicker (if running) and reset total clicks to 0.
        """
        self._clicking = False
    ...
    # ----------------------------------------
    #  Clean‐up on GUI Exit
    # ----------------------------------------
    def shutdown(self):
        """
        Called when the entire window is closing. Stops the listener and any ongoing clicking,
        and saves macros to disk.
        """
        self._clicking = False
        self._total_clicks_sent = 0
        if self._listener:
    ...
    # ----------------------------------------
    #  For GUI to Poll “Total Clicks” Every 100ms
    # ----------------------------------------
    @property
...
# ================================================
#  The Tkinter GUI Class (with Macros UI & Persistence)
# ================================================
class AutoClickerGUI:
    def __init__(self, root):
        ...
        # ==================================
        #  Frame: # clicks (→ clicks/sec), trigger, stop at
        # ==================================
        ...
        # ==================================
        #  Frame: Mouse / Key to Click (Master)
        # ==================================
        ...
        # Buttons to add / edit / remove macros
        ...
        # Instantiate the AutoClicker logic, handing it a reference to ourselves
        self.clicker = AutoClicker(self)

        # Start the periodic “pull from GUI” & GUI state-update loop
        self._poll_gui_to_clicker()

        # Handle window close for a clean shutdown
        self.root.protocol("WM_DELETE_WINDOW", self._on_window_close)

    # ----------------------------------------
    #  Periodic Polling (Every 100 ms)
    # ----------------------------------------
    def _poll_gui_to_clicker(self):
        """
        1) Read GUI fields & push into clicker.update_settings_from_gui()
        2) Enable/disable widgets based on mode & button choice
        3) Enable/disable master X/Y based on UseFixed
        4) Validate trigger key; highlight the entry if invalid
        5) Update “TOTAL:” label from clicker.total_clicks_sent
        6) Enable/disable STOP! button based on clicker._clicking
        7) Refresh the macro listbox & macro status line
        Then schedule itself 100 ms later.
        """
        # 1) Push GUI → logic
        self.clicker.update_settings_from_gui()

        # 2) Enable/disable “key” entry based on master button choice
        ...
        # 3) Enable/disable master X/Y based on use_fixed checkbox
    ...
    # ----------------------------------------
    #  Methods Called by AutoClicker to Update Status
    # ----------------------------------------
    def set_status(self, text: str):
        """
        Called by AutoClicker (master or macro) to update “status: <text>.”
        By default, if text=="idle" → green; if text=="clicking" → red.
        If text starts with “macro:”, it's orange by default.
        """
        self.status_var.set(text)

    def set_status_color(self, color_name: str):
        """
        Change the color of the status label text. E.g. "green", "red", "orange".
        """
    ...
    def _on_stop_pressed(self):
        """
        Called when you click STOP! in the GUI. Cancels master clicking & resets total.
        """
    ...
    def _on_help_pressed(self):
        """
        Pops up a message box explaining how to use everything.
        """
        msg = (
            "=== Master Clicker ===\n"
    ...
    # ============================================
    #  Macro Listbox Management
    # ============================================
    def _refresh_macro_listbox(self):
        """
        Ensure that the Listbox always matches the contents of clicker.macros.
        """
    ...
    def _on_add_macro(self):
        """
        Open a pop-up window to let the user define a new macro.
        """
        self._open_macro_editor()

    def _on_edit_macro(self):
        """
        Open the same pop-up to edit the currently selected macro.
        """
    ...
    def _on_remove_macro(self):
        """
        Remove the currently selected macro from the list, then save to disk.
        """
        sel = self.listbox_macros.curselection()
        if not sel:
            return
    ...
    def _open_macro_editor(self, edit_index=None):
        """
        If edit_index is None, we're adding a new macro. Otherwise, we're editing the macro
        at that index in self.clicker.macros.
        """
        ...
        def save_macro():
            ...
            # If button is 'key' and key_text is empty, error
            if action == "key" and not key_text:
                messagebox.showerror("Invalid Key", "Enter a key name for the 'key' action.")
                return

            # Check for duplicate trigger (unless we're editing and haven't changed it)
            ...
            if is_edit:
                # Update the existing macro in place
        ...
        # If 'key' is chosen, the user must type a keystroke name
